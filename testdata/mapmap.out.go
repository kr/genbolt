...
// Regardless, it always returns a non-nil *MapOfMapOfBool;
// if the bucket doesn't exist
// and o's transaction is read-only, the returned value
// represents an empty bucket.
func (o *T) M() *MapOfMapOfBool {
	return &MapOfMapOfBool{bucket(o.db, keyM)}
}

// MapOfMapOfBool is a bucket with arbitrary keys,
// holding child buckets of type MapOfBool.
type MapOfMapOfBool struct {
	db *bolt.Bucket
}
...
// bucket has not previously been created in a writable
// transaction, Bucket returns nil.
func (o *MapOfMapOfBool) Bucket() *bolt.Bucket {
	return o.db
}
...
// and o's transaction is read-only, the returned value
// represents an empty bucket.
func (o *MapOfMapOfBool) Get(key []byte) *MapOfBool {
	return &MapOfBool{bucket(o.db, key)}
}

// GetByString is equivalent to o.Get([]byte(key)).
func (o *MapOfMapOfBool) GetByString(key string) *MapOfBool {
	return &MapOfBool{bucket(o.db, []byte(key))}
}
